import "react-multi-date-picker/styles/backgrounds/bg-dark.css";
import "react-multi-date-picker/styles/colors/yellow.css";

import DatePicker from "react-multi-date-picker";
import { FaRegCalendarAlt } from "react-icons/fa";
import ReportsList from "./ReportsList";
import SearchInput from "../SearchInput";
import TimePicker from "react-multi-date-picker/plugins/time_picker";
import Title from "../Title";
import persian from "react-date-object/calendars/persian";
import persian_fa from "react-date-object/locales/persian_fa";
import styled from "styled-components";
import { useState } from "react";

const Container = styled.div`
  padding: 20px 15px 0px 0;
  direction: ltr;
  overflow-y: auto;
  height: 550px;
  @media (min-width: 640px) {
    height: 240px;
  }
  @media (min-width: 740px) {
    height: 225px;
  }
  @media (min-width: 840px) {
    height: 255px;
  }
  @media (min-width: 880px) {
    height: 210px;
  }
  @media (min-width: 890px) {
    height: 278px;
  }
  @media (min-width: 930px) {
    height: 294px;
  }
  @media (min-width: 1024px) {
    height: 400px;
  }
  @media (min-width: 1180px) {
    height: 600px;
  }
  @media (min-width: 1280px) {
    height: 577px;
  }
  @media (min-width: 1366px) {
    height: 650px;
  }
  @media (min-width: 1920px) {
    height: 640px;
  }
`;
const Div = styled.div`
  display: grid;
  grid-template-columns: 3fr 2fr;
  align-items: center;
  direction: rtl;
  gap: 20px;
  margin-top: 15px;
  @media (min-width: 1024px) {
    grid-template-columns: 3fr 1fr;
  }
`;

const Date = styled.div`
  direction: rtl;
  border-radius: 5px;
  border: 1px solid #454545;
  display: flex;
  flex-grow: 1;
  align-items: center;
  justify-content: space-between;
  overflow: hidden;
  background-color: #2c2c2c;
  height: 48px;
  padding: 0 10px;
  svg {
    color: gray;
  }
  input {
    color: gray;
    width: 100%;
    height: 100%;
    background-color: transparent;
    border: none;
    outline: none;
    &:focus {
      border: none;
      outline: none;
    }
    font-size: 16px;
  }
`;
const rows_items = [
  {
    id: 1,
    code: "827161",
    date: "۲۱ اردیبهشت ۱۴۰۳",
    time: " ۱۶:۲۱:۰۸",
    member: "خطا در نمایش",
    status: "confirmed",
    member_slug: "display",
    title: "لورم ایپسوم متن ساختگی با تولید سادگی نامفهوم لورم ایپسوم متن ساختگی با تولید سادگی نامفهو لورم ایپسوم متن ساختگی با تولید سادگی نامفهو",
    gif: 0.5,
    count: 100,
    psc: 1000,
  },
  {
    id: 2,
    code: "789452",
    date: "۲۱ اردیبهشت ۱۴۰۳",
    time: " ۱۶:۲۱:۰۸",
    member: "خطا در کدنویسی",
    status: "failed",
    member_slug: "code",
    title: "لورم ایپسوم متن ساختگی با تولید سادگی نامفهوم",
    gif: 1.5,
    count: 100,
    psc: 1000,
  },
  {
    id: 3,
    code: "953258",
    date: "۲۱ اردیبهشت ۱۴۰۳",
    time: " ۱۶:۲۱:۰۸",
    member: "بی احترامی",
    status: "confirmed",
    member_slug: "respect",
    title: "لورم ایپسوم متن ساختگی با تولید سادگی نامفهوم لورم ایپسوم متن ساختگی با تولید سادگی نامفهو لورم ایپسوم متن ساختگی با تولید سادگی نامفهو",
    gif: 1.6,
    count: 100,
    psc: 1000,
  },
  {
    id: 4,
    code: "135647",
    date: "۲۱ اردیبهشت ۱۴۰۳",
    time: " ۱۶:۲۱:۰۸",
    member: "خطا در کدنویسی",
    status: "pending",
    member_slug: "code",
    title: "لورم ایپسوم متن ساختگی با تولید سادگی نامفهوم",
    gif: 1.7,
    count: 100,
    psc: 1000,
  },
  {
    id: 5,
    code: "978512",
    date: "۲۱ اردیبهشت ۱۴۰۳",
    time: " ۱۶:۲۱:۰۸",
    member: "خطا در کدنویسی",
    status: "failed",
    member_slug: "code",
    title: "لورم ایپسوم متن ساختگی با تولید سادگی نامفهوم",
    gif: 1.6,
    count: 100,
    psc: 1000,
  },
  {
    id: 6,
    code: "954274",
    date: "۲۱ اردیبهشت ۱۴۰۳",
    time: " ۱۶:۲۱:۰۸",
    member: "خطا در کدنویسی",
    status: "pending",
    member_slug: "code",
    title: "لورم ایپسوم متن ساختگی با تولید سادگی نامفهوم لورم ایپسوم متن ساختگی با تولید سادگی نامفهو لورم ایپسوم متن ساختگی با تولید سادگی نامفهو",
    gif: 3.2,
    count: 100,
    psc: 1000,
  },
  {
    id: 7,
    code: "875162",
    date: "۲۱ اردیبهشت ۱۴۰۳",
    time: " ۱۶:۲۱:۰۸",
    member: "خطا در کدنویسی",
    status: "pending",
    member_slug: "code",
    title: "لورم ایپسوم متن ساختگی با تولید سادگی نامفهوم",
    gif: 1.5,
    count: 100,
    psc: 1000,
  },
  {
    id: 8,
    code: "147862",
    date: "۲۱ اردیبهشت ۱۴۰۳",
    time: " ۱۶:۲۱:۰۸",
    member: "FBS کندی سامانه",
    status: "confirmed",
    member_slug: "fbs",
    title: "لورم ایپسوم متن ساختگی با تولید سادگی نامفهوم",
    gif: 0.5,
    count: 100,
    psc: 1000,
  },
  {
    id: 9,
    code: "865721",
    date: "۲۱ اردیبهشت ۱۴۰۳",
    time: " ۱۶:۲۱:۰۸",
    member: "بی احترامی",
    status: "failed",
    member_slug: "respect",
    title: "لورم ایپسوم متن ساختگی با تولید سادگی نامفهوم",
    gif: 3.2,
    count: 100,
    psc: 1000,
  },
  {
    id: 10,
    code: "827161",
    date: "۲۱ اردیبهشت ۱۴۰۳",
    time: " ۱۶:۲۱:۰۸",
    member: "خطا در املا",
    status: "confirmed",
    member_slug: "spell",
    title: "لورم ایپسوم متن ساختگی با تولید سادگی نامفهوم لورم ایپسوم متن ساختگی با تولید سادگی نامفهو لورم ایپسوم متن ساختگی با تولید سادگی نامفهو",
    gif: 0.5,
    count: 100,
    psc: 1000,
  },
  {
    id: 11,
    code: "789452",
    date: "۲۱ اردیبهشت ۱۴۰۳",
    time: " ۱۶:۲۱:۰۸",
    member: "خطا در نمایش",
    status: "pending",
    member_slug: "display",
    title: "لورم ایپسوم متن ساختگی با تولید سادگی نامفهوم",
    gif: 1.5,
    count: 100,
    psc: 1000,
  },
  {
    id: 12,
    code: "953258",
    date: "۲۱ اردیبهشت ۱۴۰۳",
    time: " ۱۶:۲۱:۰۸",
    member: "خطا در کدنویسی",
    status: "confirmed",
    member_slug: "code",
    title: "لورم ایپسوم متن ساختگی با تولید سادگی نامفهوم",
    gif: 1.6,
    count: 100,
    psc: 1000,
  },
  {
    id: 13,
    code: "135647",
    date: "۲۱ اردیبهشت ۱۴۰۳",
    time: " ۱۶:۲۱:۰۸",
    member: "خطا در نمایش",
    status: "failed",
    member_slug: "display",
    title: "لورم ایپسوم متن ساختگی با تولید سادگی نامفهوم",
    gif: 1.7,
    count: 100,
    psc: 1000,
  },
  {
    id: 14,
    code: "978512",
    date: "۲۱ اردیبهشت ۱۴۰۳",
    time: " ۱۶:۲۱:۰۸",
    member: "بی احترامی",
    status: "confirmed",
    member_slug: "respect",
    title: "لورم ایپسوم متن ساختگی با تولید سادگی نامفهوم",
    gif: 1.6,
    count: 100,
    psc: 1000,
  },
  {
    id: 15,
    code: "954274",
    date: "۲۱ اردیبهشت ۱۴۰۳",
    time: " ۱۶:۲۱:۰۸",
    member: "FBS کندی سامانه",
    status: "pending",
    member_slug: "fbs",
    title: "لورم ایپسوم متن ساختگی با تولید سادگی نامفهوم",
    gif: 3.2,
    count: 100,
    psc: 1000,
  },
  {
    id: 16,
    code: "875162",
    date: "۲۱ اردیبهشت ۱۴۰۳",
    time: " ۱۶:۲۱:۰۸",
    member: "خطا در کدنویسی",
    status: "pending",
    member_slug: "code",
    title: "لورم ایپسوم متن ساختگی با تولید سادگی نامفهوم",
    gif: 1.5,
    count: 100,
    psc: 1000,
  },
  {
    id: 17,
    code: "147862",
    date: "۲۱ اردیبهشت ۱۴۰۳",
    time: " ۱۶:۲۱:۰۸",
    member: "خطا در کدنویسی",
    status: "confirmed",
    member_slug: "code",
    title: "لورم ایپسوم متن ساختگی با تولید سادگی نامفهوم",
    gif: 0.5,
    count: 100,
    psc: 1000,
  },
  {
    id: 18,
    code: "865721",
    date: "۲۱ اردیبهشت ۱۴۰۳",
    time: " ۱۶:۲۱:۰۸",
    member: "خطا در کدنویسی",
    status: "pending",
    member_slug: "code",
    title: "لورم ایپسوم متن ساختگی با تولید سادگی نامفهوم",
    gif: 3.2,
    count: 100,
    psc: 1000,
  },
  {
    id: 19,
    code: "827161",
    date: "۲۱ اردیبهشت ۱۴۰۳",
    time: " ۱۶:۲۱:۰۸",
    member: "خطا در نمایش",
    status: "failed",
    member_slug: "display",
    title: "لورم ایپسوم متن ساختگی با تولید سادگی نامفهوم",
    gif: 0.5,
    count: 100,
    psc: 1000,
  },
  {
    id: 20,
    code: "789452",
    date: "۲۱ اردیبهشت ۱۴۰۳",
    time: " ۱۶:۲۱:۰۸",
    member: "خطا در املا",
    status: "pending",
    member_slug: "spell",
    title: "لورم ایپسوم متن ساختگی با تولید سادگی نامفهوم",
    gif: 1.5,
    count: 100,
    psc: 1000,
  },
  {
    id: 21,
    code: "953258",
    date: "۲۱ اردیبهشت ۱۴۰۳",
    time: " ۱۶:۲۱:۰۸",
    member: "بی احترامی",
    status: "confirmed",
    member_slug: "respect",
    title: "لورم ایپسوم متن ساختگی با تولید سادگی نامفهوم",
    gif: 1.6,
    count: 100,
    psc: 1000,
  },
  {
    id: 22,
    code: "135647",
    date: "۲۱ اردیبهشت ۱۴۰۳",
    time: " ۱۶:۲۱:۰۸",
    member: "خطا در نمایش",
    status: "pending",
    member_slug: "display",
    title: "لورم ایپسوم متن ساختگی با تولید سادگی نامفهوم",
    gif: 1.7,
    count: 100,
    psc: 1000,
  },
  {
    id: 23,
    code: "978512",
    date: "۲۱ اردیبهشت ۱۴۰۳",
    time: " ۱۶:۲۱:۰۸",
    member: "خطا در کدنویسی",
    status: "confirmed",
    member_slug: "code",
    title: "لورم ایپسوم متن ساختگی با تولید سادگی نامفهوم",
    gif: 1.6,
    count: 100,
    psc: 1000,
  },
  {
    id: 24,
    code: "954274",
    date: "۲۱ اردیبهشت ۱۴۰۳",
    time: " ۱۶:۲۱:۰۸",
    member: "FBS کندی سامانه",
    status: "pending",
    member_slug: "fbs",
    title: "لورم ایپسوم متن ساختگی با تولید سادگی نامفهوم",
    gif: 3.2,
    count: 100,
    psc: 1000,
  },
  {
    id: 25,
    code: "875162",
    date: "۲۱ اردیبهشت ۱۴۰۳",
    time: " ۱۶:۲۱:۰۸",
    member: "خطا در کدنویسی",
    status: "pending",
    member_slug: "code",
    title: "لورم ایپسوم متن ساختگی با تولید سادگی نامفهوم",
    gif: 1.5,
    count: 100,
    psc: 1000,
  },
  {
    id: 26,
    code: "147862",
    date: "۲۱ اردیبهشت ۱۴۰۳",
    time: " ۱۶:۲۱:۰۸",
    member: "FBS کندی سامانه",
    status: "confirmed",
    member_slug: "fbs",
    title: "لورم ایپسوم متن ساختگی با تولید سادگی نامفهوم",
    gif: 0.5,
    count: 100,
    psc: 1000,
  },
  {
    id: 27,
    code: "865721",
    date: "۲۱ اردیبهشت ۱۴۰۳",
    time: " ۱۶:۲۱:۰۸",
    member: "خطا در کدنویسی",
    status: "pending",
    member_slug: "code",
    title: "لورم ایپسوم متن ساختگی با تولید سادگی نامفهوم",
    gif: 3.2,
    count: 100,
    psc: 1000,
  },
  {
    id: 28,
    code: "865721",
    date: "۲۱ اردیبهشت ۱۴۰۳",
    time: " ۱۶:۲۱:۰۸",
    member: "خطا در کدنویسی",
    status: "pending",
    member_slug: "code",
    title: "لورم ایپسوم متن ساختگی با تولید سادگی نامفهوم",
    gif: 3.2,
    count: 100,
    psc: 1000,
  },
  {
    id: 29,
    code: "865721",
    date: "۲۱ اردیبهشت ۱۴۰۳",
    time: " ۱۶:۲۱:۰۸",
    member: "خطا در کدنویسی",
    status: "pending",
    member_slug: "code",
    title: "لورم ایپسوم متن ساختگی با تولید سادگی نامفهوم",
    gif: 3.2,
    count: 100,
    psc: 1000,
  },
  {
    id: 30,
    code: "865721",
    date: "۲۱ اردیبهشت ۱۴۰۳",
    time: " ۱۶:۲۱:۰۸",
    member: "خطا در کدنویسی",
    status: "pending",
    member_slug: "code",
    title: "لورم ایپسوم متن ساختگی با تولید سادگی نامفهوم",
    gif: 3.2,
    count: 100,
    psc: 1000,
  },
];
const ReportsListTab = ({ title, subdomain }) => {
  
  
  const [searched, setSearched] = useState("");
  const [status, setStatus] = useState({
    pending: false,
    confirmed: false,
    failed: false,
  });

  const [member, setMember] = useState({
    respect: false,
    code: false,
    display: false,
    fbs: false,
    spell: false,
  });

  const [rows, setRows] = useState(rows_items);
  const filteredItems = rows.filter((row) => {
    const titleMatch = row.title.toString().includes(searched);
    const statusMatch =
      (!status.confirmed && !status.failed && !status.pending) ||
      (status.confirmed && row.status === "confirmed") ||
      (status.failed && row.status === "failed") ||
      (status.pending && row.status === "pending");
    const memberMatch =
      (!member.display &&
        !member.spell &&
        !member.fbs &&
        !member.respect &&
        !member.code) ||
      (member.display && row.member_slug === "display") ||
      (member.spell && row.member_slug === "spell") ||
      (member.fbs && row.member_slug === "fbs") ||
      (member.code && row.member_slug === "code") ||
      (member.respect && row.member_slug === "respect");

    return titleMatch && statusMatch && memberMatch;
  });

  return (
    <Container>
      <div dir="rtl">
        <Title title="لیست گزارشات" />
      </div>
      <Div>
        <SearchInput
          onchange={(e) => setSearched(e.target.value)}
          value={searched}
          placeholder="جستجو عنوان گزارش"
        />
        <Date>
          <DatePicker
            placeholder="تاریخ و ساعت گزارش"
            className="bg-dark yellow"
            format="YYYY/DD/MM HH:mm:ss"
            plugins={[<TimePicker position="bottom" />]}
            calendar={persian}
            locale={persian_fa}
            calendarPosition="bottom-right"
          />
          <FaRegCalendarAlt size={20} />
        </Date>
      </Div>
      <ReportsList
        setStatus={setStatus}
        setMember={setMember}
        member={member}
        status={status}
        rows={filteredItems}
        domain={title}
        subdomain={subdomain}
      />
    </Container>
  );
};

export default ReportsListTab;
